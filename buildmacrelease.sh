...
cleanup
/usr/local/share/dotnet/dotnet publish --self-contained -c Release -f net6.0 -r osx.11.0-arm64 -p:PublishReadyToRun=true -p:TieredCompilation=true
mkdir "$TEMPDIR/arm64"
cp -r "bin/Release/net6.0/osx.11.0-arm64/publish/FrankenDrift.Runner.Mac.app" "$TEMPDIR/arm64"
...
cleanup
/usr/local/share/dotnet/x64/dotnet publish --self-contained -c Release -f net6.0 -r osx-x64 -p:PublishReadyToRun=true -p:TieredCompilation=true
mkdir "$TEMPDIR/x64"
cp -r "bin/Release/net6.0/osx-x64/publish/FrankenDrift.Runner.Mac.app" "$TEMPDIR/x64"
