...
cleanup
/usr/local/share/dotnet/dotnet publish --self-contained -c Release -f net8.0 -r osx-arm64 -p:PublishReadyToRun=true -p:PublishReadyToRunComposite=true -p:TieredCompilation=true -p:Version=$BLDVER -p:IncludeSourceRevisionInInformationalVersion=false
mkdir "$TEMPDIR/arm64"
...
cleanup
/usr/local/share/dotnet/x64/dotnet publish --self-contained -c Release -f net8.0 -r osx-x64 -p:PublishReadyToRun=true -p:PublishReadyToRunComposite=true -p:TieredCompilation=true -p:Version=$BLDVER -p:IncludeSourceRevisionInInformationalVersion=false
mkdir "$TEMPDIR/x64"
